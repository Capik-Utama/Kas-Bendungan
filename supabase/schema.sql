...
begin
  ...
  if exists (select 1 from public.profiles where lower(username) = normalized) then raise exception 'Username sudah digunakan'; end if;
  ...
  values ('00000000-0000-0000-0000-000000000000', new_id, 'authenticated', 'authenticated', normalized || '@kas-bendungan.id', crypt(p_password, gen_salt('bf')), now(), '', '', '', '', '', '', '', '', '', '{"provider":"email","providers":["email"]}'::jsonb, jsonb_build_object('username', trim(p_username), 'display_name', trim(p_display_name)), now(), now());
