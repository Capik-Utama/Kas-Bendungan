...
export default function CardFolderPage() {
  ...
  return <main className="folder-shell"><div className="grain" aria-hidden="true" /><div className="folder-toolbar"><Link href="/" className="back-link">← Semua kartu</Link><span className="folder-path">Ruang pembukuan / {current.name}</span></div><header className="folder-heading"><div><span className="fund-label">FOLDER PEMBUKUAN</span><h1>{current.name}</h1><p>{current.note}</p></div>{canEdit && <div className="folder-actions"><button type="button" className="edit-card folder-edit" onClick={() => openEditMenu(current)}>Edit kartu</button><button type="button" className="delete-card folder-delete" onClick={() => void deleteCard(current)}>Hapus kelompok</button></div>}</header><div className="folder-balance"><div className="balance-top"><span>Saldo folder</span><span>Terakhir diperbarui hari ini</span></div><strong>{rupiah(current.amount)}</strong><div className="balance-bottom"><span>Kas yang tersedia di folder ini</span><span>↗</span></div></div><div className="folder-divider" /><section className="folder-section"><div className="folder-grid">{children.map((card, index) => <Link key={card.id} href={`/kartu/${card.id}`} className={`fund-card ${colors[index % colors.length]}`}><div className="fund-icon">{card.icon}</div><div className="fund-content"><span className="fund-label">{card.category.toUpperCase()}</span><h4>{card.name}</h4><p>{card.note}</p></div><div className="fund-amount">{rupiah(card.amount)}</div><span className="card-arrow">Buka ↗</span></Link>)}</div></section>{canEdit && showEditMenu && <section className="add-menu card-edit-menu"><div className="add-menu-heading"><div><span className="fund-label">PENGATURAN KARTU</span><h3>Edit kartu kelompok</h3></div><button className="panel-close" onClick={() => setShowEditMenu(false)} aria-label="Tutup pengaturan kartu">×</button></div><label className="card-setting"><span>Nama kartu</span><input value={editName} onChange={(event) => setEditName(event.target.value)} aria-label="Nama kartu" /></label>{current.category === "Kelompok" && <label className="card-setting"><input type="checkbox" checked={editAllowMembers} onChange={(event) => setEditAllowMembers(event.target.checked)} /> Izinkan penambahan anggota pada kartu ini</label>}<div className="edit-menu-actions"><button type="button" className="edit-card" onClick={() => void saveCardSettings()}>Simpan pengaturan</button><button type="button" className="panel-close" onClick={() => setShowEditMenu(false)}>Batal</button></div></section>}{canEdit && showAddMenu && <section className="add-menu"><div className="add-menu-heading"><div><span className="fund-label">FOLDER BERTINGKAT</span><h3>Tambah isi folder</h3></div><button className="panel-close" onClick={() => setShowAddMenu(false)} aria-label="Tutup menu tambah">×</button></div><label className="card-setting"><input type="checkbox" checked={allowMembers} onChange={(event) => setAllowMembers(event.target.checked)} /> Izinkan tambah anggota pada kartu Kelompok</label><div className="add-options">{addCategories.map((category) => <button key={category} onClick={() => addChild(category)}><span>{category === "Kelompok" ? "▦" : category === "Pemasukan" ? "↗" : category === "Pengeluaran" ? "↘" : category === "Anggota" ? "♙" : "▤"}</span><b>{category}</b><small>Buat kartu {category.toLowerCase()}</small></button>)}</div></section>}{canEdit && <button className="add-card folder-add-card" onClick={() => { setShowAddMenu((open) => !open); setShowEditMenu(false); }} aria-expanded={showAddMenu}><span className="plus">+</span><span><b>Tambah kartu di sini</b><small>Buat folder bertingkat di dalam {current.name}</small></span></button>}<section className="folder-caption"><span className="fund-label">ISI FOLDER</span><h3>{children.length ? `${children.length} kartu di dalam` : "Belum ada subkartu"}</h3><span className="folder-hint">Klik kartu untuk membuka folder berikutnya</span></section>{notice && <div className="toast">{notice}</div>}</main>;
}
