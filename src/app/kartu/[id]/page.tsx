...
type Card = { id: number; name: string; note: string; icon: string; amount: number; parentId: number | null; category: string; allowMembers?: boolean };
...
export default function CardFolderPage() {
  ...
  async function addChild(category: string) {
    ...
    if (!name?.trim()) return;
    const draft: Card = { id: nextLocalId++, name: name.trim(), note: `${category} pembukuan baru`, icon: "+", amount: 0, parentId: current.id, category, allowMembers };
    if (supabase) {
      const { data, error } = await supabase.from("kartu_kas").insert({ parent_id: current.id, kategori: category, nama: draft.name, catatan: draft.note, nominal: 0, ikon: "+", allow_tambah_anggota: category === "Kelompok" && allowMembers }).select("id").single();
  ...
  return <main className="folder-shell"><div className="grain" aria-hidden="true" /><div className="folder-toolbar"><Link href="/" className="back-link">← Semua kartu</Link><span className="folder-path">Ruang pembukuan / {current.name}</span></div><header className="folder-heading"><div><span className="fund-label">{current.category === "Kelompok" ? "FOLDER PEMBUKUAN" : current.category.toUpperCase()}</span><h1>{current.name}</h1><p>{current.note}</p></div>{canManageCurrent && <div className="folder-actions"><button type="button" className="edit-card folder-edit" onClick={() => openEditMenu(current)}>Edit kartu</button><button type="button" className="delete-card folder-delete" onClick={() => void deleteCard(current)}>Hapus kelompok</button></div>}</header><div className="folder-balance"><div className="balance-top"><span>Saldo folder</span><span>Terakhir diperbarui hari ini</span></div><strong>{rupiah(current.amount)}</strong><div className="balance-bottom"><span>Kas yang tersedia di folder ini</span><span>↗</span></div></div><div className="folder-divider" /><section className="folder-section"><div className="folder-grid">{children.map((card, index) => <Link key={card.id} href={functionHref(card)} className={`fund-card ${colors[index % colors.length]}`}><div className="fund-icon">{card.icon}</div><div className="fund-content"><span className="fund-label">{card.category.toUpperCase()}</span><h4>{card.name}</h4><p>{card.note}</p></div><div className="fund-amount">{rupiah(card.amount)}</div><span className="card-arrow">Buka ↗</span></Link>)}</div></section>{canManageCurrent && showEditMenu && <section className="add-menu card-edit-menu"><div className="add-menu-heading"><div><span className="fund-label">PENGATURAN KARTU</span><h3>Edit kartu kelompok</h3></div><button className="panel-close" onClick={() => setShowEditMenu(false)} aria-label="Tutup pengaturan kartu">×</button></div><label className="card-setting"><span>Nama kartu</span><input value={editName} onChange={(event) => setEditName(event.target.value)} aria-label="Nama kartu" /></label><label className="card-setting"><input type="checkbox" checked={editAllowMembers} onChange={(event) => setEditAllowMembers(event.target.checked)} /> Izinkan penambahan anggota pada kartu ini</label><div className="edit-menu-actions"><button type="button" className="edit-card" onClick={() => void saveCardSettings()}>Simpan pengaturan</button><button type="button" className="panel-close" onClick={() => setShowEditMenu(false)}>Batal</button></div></section>}{canManageCurrent && showAddMenu && <section className="add-menu"><div className="add-menu-heading"><div><span className="fund-label">FOLDER BERTINGKAT</span><h3>Tambah subkelompok</h3></div><button className="panel-close" onClick={() => setShowAddMenu(false)} aria-label="Tutup menu tambah">×</button></div><label className="card-setting"><input type="checkbox" checked={allowMembers} onChange={(event) => setAllowMembers(event.target.checked)} /> Izinkan penambahan anggota pada subkelompok ini</label><div className="add-options"><button onClick={() => void addChild("Kelompok")}><span>▦</span><b>Kelompok</b><small>Otomatis membuat kartu fungsi</small></button></div></section>}{canManageCurrent && <button className="add-card folder-add-card" onClick={() => { setShowAddMenu((open) => !open); setShowEditMenu(false); }} aria-expanded={showAddMenu}><span className="plus">+</span><span><b>Tambah subkelompok</b><small>Buat kelompok bertingkat di dalam {current.name}</small></span></button>}<section className="folder-caption"><span className="fund-label">ISI FOLDER</span><h3>{children.length ? `${children.length} kartu di dalam` : "Belum ada subkartu"}</h3><span className="folder-hint">Klik kartu fungsi untuk membuka tabel kelompok</span></section>{notice && <div className="toast">{notice}</div>}</main>;
}
