...
export default function CardFolderPage() {
  ...
  return <main className="folder-shell"><div className="grain" aria-hidden="true" /><div className="folder-toolbar"><Link href="/" className="back-link">← Semua kartu</Link><span className="folder-path">Ruang pembukuan / {current.name}</span></div><header className="folder-heading"><div><span className="fund-label">FOLDER PEMBUKUAN</span><h1>{current.name}</h1><p>{current.note}</p></div><div className="folder-total"><span>Saldo folder</span><b>{rupiah(current.amount)}</b></div></header><div className="folder-divider" /><section className="folder-section"><div className="section-heading"><div><span className="fund-label">ISI FOLDER</span><h3>{children.length ? `${children.length} kartu di dalam` : "Belum ada subkartu"}</h3></div><span className="folder-hint">Klik kartu untuk membuka folder berikutnya</span></div><div className="folder-grid">{children.map((card, index) => <Link key={card.id} href={`/kartu/${card.id}`} className={`fund-card ${colors[index % colors.length]}`}><div className="fund-icon">{card.icon}</div><div className="fund-content"><span className="fund-label">{card.category.toUpperCase()}</span><h4>{card.name}</h4><p>{card.note}</p></div><div className="fund-amount">{rupiah(card.amount)}</div><span className="card-arrow">Buka ↗</span></Link>)}<button className="add-card" onClick={() => setShowAddMenu((open) => !open)} aria-expanded={showAddMenu}><span className="plus">+</span><span><b>Tambah kartu di sini</b><small>Buat folder bertingkat di dalam {current.name}</small></span></button></div></section>{showAddMenu && <section className="add-menu"><div className="add-menu-heading"><div><span className="fund-label">FOLDER BERTINGKAT</span><h3>Tambah isi folder</h3></div><button className="panel-close" onClick={() => setShowAddMenu(false)} aria-label="Tutup menu tambah">×</button></div><div className="add-options">{addCategories.map((category) => <button key={category} onClick={() => addChild(category)}><span>{category === "Kelompok" ? "▦" : category === "Pemasukan" ? "↗" : category === "Pengeluaran" ? "↘" : category === "Anggota" ? "♙" : "▤"}</span><b>{category}</b><small>Buat kartu {category.toLowerCase()}</small></button>)}</div></section>}<section className="module-section"><span className="fund-label">MODUL FOLDER INI</span><div className="module-grid">{modules.map(([name, detail, icon, href]) => <Link className="module-card" key={name} href={href}><span className="module-icon">{icon}</span><b>{name}</b><small>{detail}</small><span className="module-arrow">↗</span></Link>)}</div></section>{notice && <div className="toast">{notice}</div>}</main>;
}
