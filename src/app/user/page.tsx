...
export default function UserPage() {
  ...
  const groupNames = ["Semua", "Kematian", "Pemuda", "Kampung", "Umum", ...groups.map((group) => group.nama).filter((name) => !["Kematian", "Pemuda", "Kampung", "Umum"].includes(name))];
  const membershipsFor = (item: Profile) => item.warga?.warga_kelompok || item.profile_kelompok || [];
  const belongsToSelectedGroup = (item: Profile) => selectedGroup === "Semua" || membershipsFor(item).some((membership) => groups.find((group) => group.id === membership.kelompok_id)?.nama === selectedGroup);
