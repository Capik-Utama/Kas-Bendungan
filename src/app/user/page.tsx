...
export default function UserPage() {
  ...
  function resetForm() { setUsername(""); setPassword(""); setRole("anggota"); setFields(emptyFields); setEditing(null); }
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (editing) {
      ...
      if (password) {
        const { error: passwordError } = await supabase.rpc("update_account_password", { p_user_id: editing.id, p_password: password });
      ...
      setNotice("Password dan data akun berhasil diperbarui."); resetForm(); await loadProfiles(); return;
    }
    ...
    const { error: createError } = await supabase.rpc("create_account", { p_username: cleanUsername, p_display_name: cleanUsername, p_password: password, p_role: role, p_nik_ktp: optional.nik_ktp, p_nik_kk: optional.nik_kk, p_nomor_hp: optional.nomor_hp });
