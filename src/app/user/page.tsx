...
import { maskFixed, maskPhone } from "@/lib/privacy";

const roles: Role[] = ["developer", "ketua", "bendahara", "anggota"];
type Group = { id: number; nama: string };
type Profile = { id: string; username: string; display_name: string; role: Role; warga_id: number | null; nik_ktp: string | null; nik_kk: string | null; nomor_hp: string | null; profile_kelompok?: { kelompok_id: number }[]; warga?: { warga_kelompok?: { kelompok_id: number }[] } | null };
...
export default function UserPage() {
  ...
  const [items, setItems] = useState<Profile[]>([]); const [groups, setGroups] = useState<Group[]>([]); const [selectedGroups, setSelectedGroups] = useState<number[]>([]); const [selectedGroup, setSelectedGroup] = useState("Semua"); const [showMembers, setShowMembers] = useState(false);
  ...
  async function saveGroups(profileId: string, wargaId: number | null = null) { if (!supabase) return null; const { error: deleteError } = await supabase.from("profile_kelompok").delete().eq("profile_id", profileId); if (deleteError) return deleteError; if (wargaId !== null) { const { error: wargaDeleteError } = await supabase.from("warga_kelompok").delete().eq("warga_id", wargaId); if (wargaDeleteError) return wargaDeleteError; } if (!selectedGroups.length) return null; const { error: insertError } = await supabase.from("profile_kelompok").insert(selectedGroups.map((kelompok_id) => ({ profile_id: profileId, kelompok_id }))); if (insertError) return insertError; if (wargaId !== null) { const { error: wargaInsertError } = await supabase.from("warga_kelompok").insert(selectedGroups.map((kelompok_id) => ({ warga_id: wargaId, kelompok_id }))); if (wargaInsertError) return wargaInsertError; } return null; }
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (editing) {
      ...
      const groupError = await saveGroups(editing.id, editing.warga_id); if (groupError) { setError(groupError.message); return; }
  ...
  const groupNames = ["Semua", ...groups.map((group) => group.nama)];
  const activeSelectedGroup = selectedGroup !== "Semua" && groups.some((group) => group.nama === selectedGroup) ? selectedGroup : "Semua";
  const membershipsFor = (item: Profile) => item.warga?.warga_kelompok || item.profile_kelompok || [];
  const belongsToSelectedGroup = (item: Profile) => activeSelectedGroup === "Semua" || membershipsFor(item).some((membership) => groups.find((group) => group.id === membership.kelompok_id)?.nama === activeSelectedGroup);
  const visibleStaff = items.filter((item) => profile?.role === "developer" ? item.role !== "anggota" : item.role !== "developer" && item.role !== "anggota");
  const visibleMembers = items.filter((item) => item.role === "anggota" && belongsToSelectedGroup(item));
  const groupLabel = (item: Profile) => membershipsFor(item).map((membership) => groups.find((group) => group.id === membership.kelompok_id)?.nama).filter(Boolean).join(", ") || "—";
  const accountRow = (item: Profile) => <tr key={item.id}><td>{item.username}</td><td><span className={`role-pill role-${item.role}`}>{roleLabel(item.role)}</span></td><td>{groupLabel(item)}</td><td>{profile?.role === "anggota" ? maskFixed(item.nik_ktp, 6) : (item.nik_ktp || "—")}</td><td>{profile?.role === "anggota" ? maskFixed(item.nik_kk, 6) : (item.nik_kk || "—")}</td><td>{profile?.role === "anggota" ? maskPhone(item.nomor_hp) : (item.nomor_hp || "—")}</td><td>{canEditAccounts && canEditTarget(item) ? <button className="edit-user" onClick={() => { setEditing(item); setUsername(item.username); setPassword(""); setRole(item.role); setFields({ nik_ktp: item.nik_ktp || "", nik_kk: item.nik_kk || "", nomor_hp: item.nomor_hp || "" }); setSelectedGroups(membershipsFor(item).map((membership) => membership.kelompok_id)); }}>Edit</button> : <span className="read-only-label">Lihat saja</span>}</td></tr>;
  const accountTable = (list: Profile[], emptyText: string) => <div className="user-table-wrap"><table className="user-table"><thead><tr><th>Username</th><th>Role</th><th>Kelompok</th><th>NIK KTP</th><th>NIK KK</th><th>No. HP</th><th>Aksi</th></tr></thead><tbody>{list.map(accountRow)}{list.length === 0 && <tr><td colSpan={7} className="empty-table-note">{emptyText}</td></tr>}</tbody></table></div>;
  return <main className="management-shell"><div className="management-heading"><div><span className="fund-label">MANAJEMEN AKSES</span><h1>User</h1><p>Pengelola disematkan di atas. Anggota dikelompokkan dan dapat ditampilkan sesuai pilihan.</p></div></div>{profile?.role === "anggota" && <form onSubmit={onOwnPasswordSubmit} className="user-form"><h2>Ubah password sendiri</h2><div className="user-form-grid"><label>Password baru<input required type="password" minLength={8} value={ownPassword} onChange={(event) => setOwnPassword(event.target.value)} placeholder="Minimal 8 karakter" /></label></div><div className="user-actions"><button type="submit">Simpan password</button></div></form>}{canCreateAccounts && <form onSubmit={onSubmit} className="user-form"><h2>{editing ? "Edit akun" : "Tambah akun baru"}</h2><div className="user-form-grid"><label>Username<input required disabled={Boolean(editing)} value={username} onChange={(event) => setUsername(event.target.value)} placeholder="contoh: firda" /></label><label>{editing ? "Password baru" : "Password"} <span className="optional-label">{editing ? "kosongkan jika tetap" : "wajib"}</span><input required={!editing} type="password" minLength={8} value={password} onChange={(event) => setPassword(event.target.value)} placeholder={editing ? "Tidak diubah jika kosong" : "Minimal 8 karakter"} /></label><label>Role<select disabled={Boolean(editing && editing.id === profile?.id)} value={role} onChange={(event) => setRole(event.target.value as Role)}>{allowedRoles.map((item) => <option key={item} value={item}>{roleLabel(item)}</option>)}</select></label>{optionalFields}{groupChecklist}</div><div className="user-actions"><button type="submit">{editing ? "Simpan perubahan" : "Buat akun"}</button>{editing && <button type="button" className="secondary-action" onClick={resetForm}>Batal</button>}</div></form>}{notice && <p className="success-note">{notice}</p>}{error && <p className="error-note">{error}</p>}<section className="pinned-accounts"><div className="section-heading"><div><span className="fund-label">AKUN PENGELOLA</span><h2>Pengelola</h2></div><small>{profile?.role === "developer" ? "Developer dapat melihat seluruh akun pengelola." : "Akun developer disembunyikan."}</small></div>{accountTable(visibleStaff, "Belum ada akun pengelola.")}</section><section className="member-directory"><div className="member-directory-head"><div><span className="fund-label">DAFTAR ANGGOTA</span><h2>Anggota</h2></div><button className="edit-user" type="button" onClick={() => setShowMembers((current) => !current)}>{showMembers ? "Sembunyikan anggota" : "Tampilkan anggota"}</button></div>{showMembers && <><div className="group-tabs" role="tablist" aria-label="Filter kelompok">{groupNames.map((name) => <button key={name} type="button" className={activeSelectedGroup === name ? "selected" : ""} onClick={() => setSelectedGroup(name)}>{name}</button>)}</div>{accountTable(visibleMembers, `Belum ada anggota di kelompok ${activeSelectedGroup.toLowerCase()}.`)}</>}</section></main>;
}
