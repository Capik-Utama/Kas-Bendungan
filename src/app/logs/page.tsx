...
export default function LogsPage() {
  ...
  return <main className="management-shell space-y-2">
    ...
    <div className="overflow-x-auto rounded-xl border border-zinc-200 bg-white shadow-sm"><table className="min-w-[760px] w-full text-xs"><thead className="bg-zinc-100 text-zinc-700"><tr><th className="px-2 py-1.5 text-left">Waktu</th><th className="px-2 py-1.5 text-left">Aktivitas</th><th className="px-2 py-1.5 text-left">Kategori</th><th className="px-2 py-1.5 text-left">Petugas</th><th className="px-2 py-1.5 text-left">Objek</th></tr></thead><tbody>{filtered.map((item) => <tr key={item.id} className="border-t border-zinc-200 align-top"><td className="whitespace-nowrap px-2 py-1.5">{formatDate(item.created_at)}</td><td className="px-2 py-1.5 font-medium">{activityLabel(item)}</td><td className="px-2 py-1.5"><span className="rounded-full bg-zinc-100 px-1.5 py-0.5 text-[10px] font-medium">{item.category}</span></td><td className="px-2 py-1.5">{actorName(item.actor)}</td><td className="px-2 py-1.5">{item.entity ? `${item.entity}${item.entity_id ? ` #${item.entity_id}` : ""}` : "-"}</td></tr>)}{filtered.length === 0 && <tr><td colSpan={5} className="px-2 py-4 text-center text-zinc-500">Belum ada aktivitas pada filter ini.</td></tr>}</tbody></table></div>
