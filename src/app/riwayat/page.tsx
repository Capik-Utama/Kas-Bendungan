...
type RelatedRecord = { nama?: string; username?: string; display_name?: string } | { nama?: string; username?: string; display_name?: string }[] | null;

function namaWarga(value: RelatedWarga) {
  return Array.isArray(value) ? value[0]?.nama ?? "" : value?.nama ?? "";
}

function namaPetugas(value: RelatedRecord) {
  const record = Array.isArray(value) ? value[0] : value;
  return record?.display_name || record?.username || "Belum tercatat";
}

function namaKelompok(value: RelatedRecord) {
  const record = Array.isArray(value) ? value[0] : value;
  return record?.nama || "Tanpa kelompok";
}
...
export default function RiwayatPage() {
  ...
  const [sortKey, setSortKey] = useState<"nama" | "tipe" | "tanggal" | "nominal" | "sumber" | "keterangan" | "kelompok" | "petugas">("tanggal");
  ...
  const [officerFilter, setOfficerFilter] = useState("Semua");
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      const [iuranResult, pengeluaranResult, groupResult] = await Promise.all([
        client.from("iuran").select("id, tanggal_bayar, nominal, bulan, keterangan, kartu_id, warga:warga_id(nama), kelompok:kartu_id(nama), petugas:petugas_id(username,display_name)"),
        client.from("pengeluaran").select("id, tanggal, nominal, keperluan, keterangan, kartu_id, kelompok:kartu_id(nama), petugas:petugas_id(username,display_name)"),
  ...
  return <section className="space-y-4">
    ...
    <div className="flex flex-wrap items-end gap-2 rounded-xl border border-zinc-200 bg-white p-3 shadow-sm"><label className="grid gap-1 text-xs font-medium text-zinc-600">Kelompok<select value={groupFilter} onChange={(event) => { setGroupFilter(event.target.value); setPage(1); }} className="rounded-lg border border-zinc-300 px-2 py-1.5 text-sm text-zinc-800">{groupOptions.map((option) => <option key={option} value={option}>{option}</option>)}</select></label><label className="grid gap-1 text-xs font-medium text-zinc-600">Jenis<select value={typeFilter} onChange={(event) => { setTypeFilter(event.target.value as "SEMUA" | "MASUK" | "KELUAR"); setPage(1); }} className="rounded-lg border border-zinc-300 px-2 py-1.5 text-sm text-zinc-800"><option value="SEMUA">Pemasukan & Pengeluaran</option><option value="MASUK">Pemasukan</option><option value="KELUAR">Pengeluaran</option></select></label><label className="grid gap-1 text-xs font-medium text-zinc-600">Petugas<select value={officerFilter} onChange={(event) => { setOfficerFilter(event.target.value); setPage(1); }} className="rounded-lg border border-zinc-300 px-2 py-1.5 text-sm text-zinc-800">{officerOptions.map((option) => <option key={option} value={option}>{option}</option>)}</select></label></div>
    ...
    <div className="overflow-x-auto rounded-2xl border border-zinc-200 bg-white shadow-sm"><table className="min-w-[980px] text-left text-sm"><thead className="bg-zinc-100 text-zinc-700"><tr>{([ ["nama", "Nama (jika iuran)"], ["tipe", "Tipe"], ["tanggal", "Tanggal"], ["nominal", "Nominal"], ["sumber", "Sumber/Keperluan"], ["kelompok", "Kelompok"], ["petugas", "Petugas"], ["keterangan", "Keterangan"] ] as const).map(([key, label]) => <th key={key} className="whitespace-nowrap px-4 py-3"><button type="button" onClick={() => toggleSort(key)} className="font-semibold hover:text-emerald-700 print:pointer-events-none">{label}{sortLabel(key)}</button></th>)}</tr></thead><tbody>{visibleItems.map((item) => <tr key={item.id} className="border-t border-zinc-200 align-top"><td className="whitespace-nowrap px-4 py-3">{item.nama || "-"}</td><td className="whitespace-nowrap px-4 py-3"><span className={`rounded-full px-2 py-0.5 text-xs font-medium ${item.tipe === "MASUK" ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}>{item.tipe}</span></td><td className="whitespace-nowrap px-4 py-3">{formatTanggal(item.tanggal)}</td><td className="whitespace-nowrap px-4 py-3">{formatRupiah(item.nominal)}</td><td className="whitespace-nowrap px-4 py-3">{item.sumber}</td><td className="whitespace-nowrap px-4 py-3">{item.kelompok}</td><td className="whitespace-nowrap px-4 py-3">{item.petugas}</td><td className="px-4 py-3">{item.keterangan}</td></tr>)}{visibleItems.length === 0 && <tr><td colSpan={8} className="px-4 py-8 text-center text-zinc-500">Belum ada transaksi pada periode ini.</td></tr>}</tbody></table></div>
